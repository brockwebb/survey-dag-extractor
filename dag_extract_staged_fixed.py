# dag_extract_staged_fixed.py
# FIXED VERSION: Proper Stage 0 implementation using dedicated stage0_full_index module
...
from stage0_full_index import run_stage0_full_index  # FIXED: Use proper Stage 0 module
...
def main():
    ap = argparse.ArgumentParser(description="Staged Survey DAG Extraction (FIXED Stage 0)")
    ...
    log("────────────────────────────────────────────────────────")
    log("Survey DAG staged extraction (FIXED)")
    ...
    if args.stage0:
        ...
        # FIXED: Use the proper stage0_full_index function
        t0 = time.perf_counter()
        try:
            stage0_seeds = run_stage0_full_index(
                full_text=full_text,
                prompt_path=prompt_s0,
                model_id=args.s0_model
            )
            ...
            dt = time.perf_counter() - t0
            log(f"  Stage0 FAILED: {e} ↳ {dt:.1f}s")
            stage0_seeds = []  # Continue without Stage 0 seeds
